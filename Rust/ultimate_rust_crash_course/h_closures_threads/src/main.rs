...
fn main() {
    // The code below could be typed explicitly, but it can also guess the value from its user
    // later on (on sender.send), so it's not needed. That's how it would look if it was used
    // explicitly:
    // let (tx, rx):(Sender<&str>, Receiver<&str>) = channel::unbounded();
    ...
    let random_numbers = get_random_numbers();

    let random_numbers_for_first_sender = random_numbers.clone();
    let first_handle = thread::spawn(move || {
        random_numbers_for_first_sender.iter().for_each(|number| {
            pause_ms(5);
            sender
                .send(std::format!("First Thread: {}", number))
                .unwrap()
        });
    });

    let second_handle = thread::spawn(move || {
        random_numbers.iter().for_each(|number| {
            pause_ms(5);
            second_sender
                .send(std::format!("Second Thread: {}", number))
                .unwrap()
        });
    });
